...
# List of dangerous commands to block (macOS-specific)
BLOCKED_COMMANDS = [
    "rm -rf /", "shutdown", "reboot", "mkfs.ext4", "curl -O malware.sh", "wget http",
    "chmod 777", "chown root", "rm -rf *", "dd if=/dev/zero of=/dev/sda", "reboot", "poweroff",
    "diskutil eraseDisk", "systemsetup -setrestartfreeze"
]

def get_ai_command(user_input):
    """Sends user input to Gemini API and retrieves a command"""
    prompt = f"""
    You are a command-line AI assistant. Generate a single valid macOS command.
    STRICT RULES:
    - Only return the command.
    - No explanations or alternatives.
    - Ensure it runs in a macOS terminal.
